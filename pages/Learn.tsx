    ...
    <PageWrapper>
      <div className="max-w-4xl mx-auto px-6 py-12 md:py-24">
        ...
        <section className="mb-24">
          ...
          <h2 className="text-3xl font-bold text-[#1d1d1f] mb-6">What is a Deadlock?</h2>
          <div className="space-y-6 text-[#515154] leading-relaxed text-base md:text-lg font-light">
            ...
            <p className="p-6 bg-primary/[0.04] border border-primary/20 rounded-2xl italic font-normal text-primary">"In our Smart City: if Hospital AI holds Power and waits for Bandwidth, while Power Grid holds Bandwidth and waits for Power — neither can proceed. The entire city grid freezes."</p>
          </div>
        </section>
        ...
        <section className="mb-24">
          ...
          <h2 className="text-3xl font-bold text-[#1d1d1f] mb-8">Four Necessary Conditions</h2>
          <div className="grid md:grid-cols-2 gap-4">
            {[
              ...
              <div key={i} className="glass-panel p-6 border-border-dim/50 bg-white">
                <div className="flex items-center gap-3 mb-4">
                  <item.icon className="w-5 h-5 text-primary" />
                  <h4 className="text-sm font-semibold text-[#1d1d1f] tracking-tight">{item.title}</h4>
                </div>
        ...
        <section className="mb-24">
          ...
          <h2 className="text-3xl font-bold text-[#1d1d1f] mb-8">Deadlock vs Starvation</h2>
          <div className="glass-panel overflow-hidden border-border-dim/50 bg-white">
            <table className="w-full text-left text-[13px]">
              <thead>
                <tr className="bg-primary/[0.03] text-primary uppercase text-[10px] font-bold tracking-widest border-b border-border-dim/40">
              ...
              <tbody className="text-[#515154]">
                {[
                  { name: 'Deadlock', state: 'Blocked', progress: 'None', fix: 'Reset', color: 'text-secondary font-semibold' },
                  { name: 'Livelock', state: 'Active', progress: 'None', fix: 'Random Retry', color: 'text-secondary/85 font-semibold' },
                  { name: 'Starvation', state: 'Waiting', progress: 'Delayed', fix: 'Priority Aging', color: 'text-primary font-semibold' },
                  { name: 'Safe State', state: 'Running', progress: 'Safe', fix: 'Banker Check', color: 'text-success font-semibold' },
                ].map((row, i) => (
                  <tr key={i} className="border-b border-border-dim/10">
                    <td className="p-4 font-bold text-[#1d1d1f]">{row.name}</td>
                    <td className={`p-4 ${row.color}`}>{row.state}</td>
        ...
        <section className="mb-24">
          ...
          <h2 className="text-3xl font-bold text-[#1d1d1f] mb-8">Famous Deadlocks in History</h2>
          <div className="space-y-4">
            {[
              ...
              <div key={i} className="glass-panel p-6 border-l-4 border-l-primary bg-white">
        ...
        <section className="mt-32">
          ...
          <h2 className="text-3xl font-bold text-[#1d1d1f] mb-8">Quick Quiz</h2>
          
          {score !== null ? (
            <motion.div 
              initial={{ opacity: 0, scale: 0.95 }}
              animate={{ opacity: 1, scale: 1 }}
              className="glass-panel p-12 text-center space-y-6 bg-white"
            >
              <div className="w-16 h-16 bg-primary/10 rounded-full flex items-center justify-center mx-auto border border-primary/20">
                <Trophy className="w-8 h-8 text-primary" />
              </div>
              <div>
                <div className="text-5xl font-black text-[#1d1d1f]">{score}/5</div>
                <p className="text-[#86868b] mt-2 font-bold uppercase tracking-widest text-[10px]">Your Final Score</p>
              </div>
              <p className="text-[#515154] max-w-sm mx-auto">
            ...
            <div className="space-y-8">
              <div className="glass-panel p-8 bg-white">
                <div className="text-[10px] font-mono text-[#86868b] mb-4 uppercase tracking-[0.15em]">Question {quizIdx + 1} / {QUIZ.length}</div>
                <h3 className="text-lg md:text-xl text-[#1d1d1f] font-bold mb-8 leading-tight">{QUIZ[quizIdx].q}</h3>
                  ...
                  {QUIZ[quizIdx].opts.map((opt, i) => {
                    ...
                    return (
                      <button
                        ...
                        disabled={isAnswered}
                        className={`p-4 rounded-xl text-left text-xs font-semibold transition-all border flex justify-between items-center ${
                          isAnswered 
                            ? (isCorrect ? 'bg-success/5 border-success text-success' : (isSelected ? 'bg-secondary/5 border-secondary text-secondary' : 'bg-transparent border-neutral-200 text-[#515154] opacity-50'))
                            : 'bg-transparent border-neutral-200 text-[#515154] hover:border-primary hover:text-primary'
                        }`}
                      >
                        {opt}
